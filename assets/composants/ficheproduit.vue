<template>
  ...
  <div class="product-container" v-if="product">
     ...
     <div class="product-details">
        ...
        <p>Description :</p>
      <p v-html="afficherTexteComplet ? product.description : descriptionTronquee"></p>
      <a v-if="!afficherTexteComplet" href="#" @click.prevent="afficherTexteComplet = true" class="lien-toggle">
  Lire la suite
</a>
<a v-else href="#" @click.prevent="afficherTexteComplet = false" class="lien-toggle">
  Réduire
</a>
     </div>
  </div>
...
<script>
  ...
  export default {
    ...
    setup() {
      ...
      const afficherTexteComplet = ref(false);
  ...
  try {
    ...
    watch(() => product.value, (nouveauProduit) => {
      ...
      return {afficherTexteComplet,estDansLesFavoris, ajouterAuxFavoris, messageStatus ,product,formatDate,ajouterAuPanier,utilisateurAuthentifie, messagePanier  , promo ,loading, error,calculerTempsRestant,isPromoActive ,selectImage, selectedImage, onSwiper, onSlideChange,};
    },
    computed: {
    quantiteClass() {
...
.lien-toggle:hover {
  color: #FFA500; /* Une nuance plus claire ou différente au survol, si souhaité */
}
</style>